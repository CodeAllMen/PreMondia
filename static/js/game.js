...
//登录按钮新版测试，不需要注册
    $("#checkFirstLogin").click(function(){
        ...
        var password=$("#userPasswordOne").val();
        if(name==""||name==null){
            $("#loginError").text("Phone Number cannot be empty!");
        }else if(password==""||password==null){
            $("#loginError").text("password name cannot be empty!");
        }else {
            var userData = {"name": name, "password": password};
            $.ajax({
                url: 'http://www.gogamehub.com/login',
                type: 'post',
                data: JSON.stringify(userData),
                dataType: "json",
                async: false,
                success: function (result) {
                    var data = result;
                    if (data.code == "0") {
                        $("#checkFirstLogin").removeAttr("href");
                        $("#loginError").text("Phone Number or password incorrect!");
                    } else if (data.code == "1") {
                        var dataTwo = data.data;
                        $("#checkFirstLogin").removeAttr("href");
                        var b = '<div class="signTwo"><a href="#" class="btn showAll smallScreen" onclick="personal()">' +
...
/* 新的类似于注册，设置新密码和邮箱的弹出框*/
    $("#setInformation").click(function(){
        ...
        if(passwordOne==""||passwordTwo==""){
            $("#setError").text("Password cannot be empty!");
        }else if(passwordOne!=passwordTwo){
            $("#setError").text("The two password must be consistent");
        }else if(!reg.test(email)){
            $("#setError").text("Please fill in the correct mailbox!");
        }else{
            var userData={"passwd":passwordOne,"email":email,"userName":userNameTwo};
            $.ajax({
                url: 'http://www.gogamehub.com/register',
                type: 'post',
                data: JSON.stringify(userData),
                dataType: "json",
                success: function(result){
...
//home列表的点击
    ...
    $("#demoGame").click(function(){
       ...
       var userData={"game_id":a,"type":"demo"};
        $.ajax({
            url: 'http://www.gogamehub.com/buygame',
            type: 'post',
            data: JSON.stringify(userData),
            dataType: "json",
            success: function(result){
...
function loginOrNulogin(id){
   ...
   $("#game_idDemo").val(id);
   $.ajax({
        url: 'http://www.gogamehub.com/checkplay?Game_id=' + id,
       async: false,
        type: 'GET',
        // data: chargeViewDatas,
        dataType: "json",
...
function personal(){
    $.ajax({
        url: 'http://www.gogamehub.com/personal',
        type: 'GET',
        // data: chargeViewDatas,
        dataType: "json",
        success: function(result){
...
function gameLogin(){
    $.ajax({
        ...
        success: function(result){
            var data=result;
            if(data.code=="0"){
              var a='<a id="modal-801879" href="#modal-container-801879" role="button" class="btn signButton" data-toggle="modal" onclick="errorClear()"><div class="icon icon-sign" ></div></a>';
                $("#sign").html(a);
            }else if(data.code=="1") {
                var dataTwo = data.data;
                var b = '<div class="signTwo"><a href="#" class="btn showAll smallScreen" onclick="personal()">' +
...
function logout(){
    $.ajax({
        url: 'http://www.gogamehub.com/logout',
        type: 'GET',
        // data: chargeViewDatas,
        dataType: "json",
        success: function(result){
